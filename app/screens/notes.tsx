...
export default function NotesScreen({ code }: NotesProps) {
    ...
    const noteRef = ref(rdb, `rooms/${code}/notes`)


    useEffect(() => {
        if (!code) return
        const handleNewMessage = (snapshot: any) => {
            const newNote = snapshot.val() as Note

            setNotes(prevNotes => {
                const alreadyExists = prevNotes.some(note => note.id === newNote.id)
                if (alreadyExists) return prevNotes
                return [...prevNotes, newNote]
            })
        }

        const unsubscribe = onChildAdded(noteRef, handleNewMessage)
